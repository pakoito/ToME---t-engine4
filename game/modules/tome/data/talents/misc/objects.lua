...
newTalent{
	name = "Incinerating Curtain", short_name="INC_CURTAIN",
	type = {"spell/objects",1},
	points = 1,
	cooldown = 20,
	no_npc_use = true,
	...
	radius = 2,
	target = function(self, t)
		return {type="cone", range=self:getTalentRange(t), radius=self:getTalentRadius(t), selffire=true, talent=t, cone_angle=180}
	end,
	...
	action = function(self, t)
		...
		self:project(tg, x, y, function(px, py)
		if not game.level.map:checkAllEntities(px, py, "block_move") and (core.fov.distance(self.x, self.y, px, py) == self:getTalentRadius(t)) then
			local g = terrains.LAVA_FLOOR:clone()
				g:resolve() g:resolve(nil, true)
				g.temporary = t.getDuration(self,t)
				g.x = px g.y = py
				g.canAct = false
				g.energy = { value = 0, mod = 1 }
				g.old_feat = game.level.map(px, py, engine.Map.TERRAIN)
				g.useEnergy = mod.class.Trap.useEnergy
				g.dam=self:spellCrit(t.getDamage(self, t))
				g.block_projectiles=true
				g.act = function(self)
					self:useEnergy()
	...
	info = function(self, t)
		...
		return ([[Create a wall of fire in a semicircle of radius %d, blocking all projectiles and dealing %0.2f fire damage to all who cross over it. The fire will last %d turns.]]):format(radius, damDesc(self, DamageType.FIRE, damage), duration)
	end,
}