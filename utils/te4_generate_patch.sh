...
oldv=`ls t-engine4-$os-$old/game/modules/tome-*.team | head -n1 | sed -e 's/^.*tome-//' -e 's/\.team$//'`
newv=`ls t-engine4-$os-$new/game/modules/tome-*.team | head -n1 | sed -e 's/^.*tome-//' -e 's/\.team$//'`
...
echo "SQL:"
echo "REPLACE INTO patch_chain SET os='$os', vfrom='$oldv', vto='$newv', file='$os-patch-$oldv-to-$newv.zip', md5='$pmd5';"